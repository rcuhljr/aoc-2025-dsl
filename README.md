...
* `DEBUG [state]` - `ON`|`OFF` - Enable/Disable engine debugging at the current script point
...
* `PASSWORD_MODE [mode]` - `PRIMARY`|`ALTERNATE` - primary counting mode only counts exact stops on zero, alternate is any click that moves to zero
